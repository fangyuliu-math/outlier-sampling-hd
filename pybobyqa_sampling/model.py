...
class Model(object):
    def __init__(self, npt, x0, f0, xl, xu, projections, f0_nsamples, nsamples_aggregator, n=None, abs_tol=-1e20, precondition=True, do_logging=True):
        if n is None:
            n = len(x0)
    ...
    def change_point(self, k, x, f_list, allow_kopt_update=True):
        # Update point k to x (w.r.t. xbase), with objective value estimates in f_list
        assert 0 <= k < self.npt(), "Invalid index %g" % k

        self.points[k, :] = x.copy()
        self.f_values[k] = f_list if type(f_list) == float else self.nsamples_aggregator(f_list)
        self.nsamples[k] = 1 if type(f_list) == float else len(f_list)
        self.factorisation_current = False
