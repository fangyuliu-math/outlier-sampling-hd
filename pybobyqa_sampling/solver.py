...
def solve_main(objfun, x0, args, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns_so_far, nf_so_far, nx_so_far, nsamples, params,
               diagnostic_info, scaling_changes, nsamples_aggregator, f0_avg_old=None, f0_nsamples_old=None, do_logging=True, print_progress=False):
    ...
    if f0_avg_old is None:
        ...
        f0_avg = nsamples_aggregator(f_list[:num_samples_run])
    ...
    control = Controller(objfun, x0, args, f0_avg, num_samples_run, xl, xu, projections, npt, rhobeg, rhoend, nf, nx,
                         maxfun, params, scaling_changes, nsamples_aggregator, do_logging=do_logging)
    ...
    while True:
        ...
        if dnorm < params("general.safety_step_thresh") * control.rho:
            ...
            if control.rho > rhoend:
                ...
                if params("restarts.use_restarts") and params("restarts.use_soft_restarts"):
                    ...
                    if num_samples_run > 0:
                            control.model.save_point(x, nsamples_aggregator(f_list[:num_samples_run]), num_samples_run, x_in_abs_coords=True)
            ...
            control.model.change_point(knew, xnew, f_list[:num_samples_run])  # expect step, not absolute x
...
def solve(objfun, x0, args=(), bounds=None, projections=None, npt=None, rhobeg=None, rhoend=1e-8, maxfun=None, nsamples=None, user_params=None,
    ...
    xmin, fmin, gradmin, hessmin, nsamples_min, nf, nx, nruns, exit_info, diagnostic_info = \
        solve_main(objfun, x0, args, xl, xu, projections, npt, rhobeg, rhoend, maxfun, nruns, nf, nx, nsamples, params,
                    diagnostic_info, scaling_changes, nsamples_aggregator, do_logging=do_logging, print_progress=print_progress)
        ...
        if params("restarts.hard.use_old_fk"):
            xmin2, fmin2, gradmin2, hessmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, args, xl, xu, projections, npt, _rhobeg, _rhoend, maxfun, nruns, nf, nx, nsamples, params,
                            diagnostic_info, scaling_changes, nsamples_aggregator, f0_avg_old=fmin, f0_nsamples_old=nsamples_min, do_logging=do_logging, print_progress=print_progress)
        else:
            xmin2, fmin2, gradmin2, hessmin2, nsamples2, nf, nx, nruns, exit_info, diagnostic_info = \
                solve_main(objfun, xmin, args, xl, xu, projections, npt, _rhobeg, _rhoend, maxfun, nruns, nf, nx, nsamples, params,
                           diagnostic_info, scaling_changes, nsamples_aggregator, do_logging=do_logging, print_progress=print_progress)
