...
class Controller(object):
    def __init__(self, objfun, x0, args, f0, f0_nsamples, xl, xu, projections, npt, rhobeg, rhoend, nf, nx, maxfun, params, scaling_changes, nsamples_aggregator, do_logging=True):
        self.objfun = objfun
        self.maxfun = maxfun
        self.args = args
        self.nsamples_aggregator = nsamples_aggregator
        self.model = Model(npt, x0, f0, xl, xu, projections, f0_nsamples, nsamples_aggregator, abs_tol=params("model.abs_tol"),
        ...
        for k in range(1, num_directions + 1):
            ...
            if exit_info is not None:
                if num_samples_run > 0:
                    self.model.save_point(x, self.nsamples_aggregator(f_list[:num_samples_run]), num_samples_run,
                                          x_in_abs_coords=True)
                return exit_info  # return & quit

            # Otherwise, add new results (increments model.npt_so_far)
            self.model.change_point(k, x - self.model.xbase, f_list[:num_samples_run])  # expect step, not absolute x
    ...
    def initialise_random_directions(self, number_of_samples, num_directions, params):
        ...
        if params("init.run_in_parallel"):
            ...
            for ndirns in range(num_directions):
                ...
                # Otherwise, add new results (increments model.npt_so_far)
                self.model.change_point(1 + ndirns, x - self.model.xbase, f_list[:num_samples_run])  # expect step, not absolute x
                # for i in range(1, num_samples_run):
    ...
    def initialise_general_constraints(self, number_of_samples, num_directions, params):
        ...
        for k in range(1, num_directions + 1):
            ...
            if exit_info is not None:
                if num_samples_run > 0:
                    self.model.save_point(x, self.nsamples_aggregator(f_list[:num_samples_run]), num_samples_run, x_in_abs_coords=True)
                return exit_info  # didn't fix geometry - return & quit
    ...
    def geometry_step(self, knew, adelt, number_of_samples, params):
        ...
        # Otherwise, add new results
        self.model.change_point(knew, xnew, f_list[:num_samples_run])  # expect step, not absolute x
        # for i in range(1, num_samples_run):
        #     self.model.add_new_sample(knew, f_extra=f_list[i])

        # Estimate actual reduction to add to diffs vector
        f = self.nsamples_aggregator(f_list[:num_samples_run])  # estimate actual objective value
    ...
    def evaluate_objective(self, x, number_of_samples, params):
        ...
        if num_samples_run > 0 and self.nsamples_aggregator(f_list[:num_samples_run]) <= self.model.min_objective_value():
    ...
    def calculate_ratio(self, current_iter, f_list, d, gopt, H):
        exit_info = None
        f = self.nsamples_aggregator(f_list)  # estimate actual objective value
        pred_reduction = - model_value(gopt, H, d)
